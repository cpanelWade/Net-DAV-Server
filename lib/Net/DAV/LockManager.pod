...
This document describes Net::DAV::LockManager version 0.030
...
Required parameter specifying the path as a string. In the case of a directory,
the path should not end with a final '/' character.
...
Optional parameter specifying the token returned when an ancestor of the
the specified resource was locked.
